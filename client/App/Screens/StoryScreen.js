...
const StoryScreen = ({ navigation }) => {
  ...
  const [isStoryCaptionDownloaded, setIsStoryCaptionDownloaded] = useState(false)
  const [isStoryImageDownloading, setIsStoryImageDownloading] = useState(false)
  const [isStoryCaptionDownloading, setIsStoryCaptionDownloading] = useState(false)
  ...
  return (
    <View style={styles.container}>
      {isStoryCaptionDownloaded && isStoryImageDownloaded ? (
        <ProgressBar onCompletion={() => navigation.navigate('screen4')} />
      ) : null}
      <Image style={styles.imageContainer} source={{ uri: `file://${storyImageDetails.path}` }} />
